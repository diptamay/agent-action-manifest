...
The implementation counts distinct authenticated observation IDs and records hold revision. Lifecycle feedback instead counts distinct execution incidents. The host must map repeated campaign observations to incidents if semantic deduplication is required. The reference does not detect coordinated behavior, determine whether evidence is true, infer intent, or import a vendor-specific threat taxonomy.

Multiple holds compose. A campaign-context digest mismatch invalidates an unexecuted Manifest while the active hold set differs. Restoration requires an authenticated allowed principal, an active hold at the exact expected revision, a reason and an evidence reference. It does not expand Profile authority, clear an action reservation or schedule a retry.

**Current campaign restoration limitations.** `ManagedGate.restore_restriction` blocks matching `DISPATCHED` or `EXECUTION_UNKNOWN` executions and advances a feedback epoch. `V4ManagedGate.restore_campaign_hold` does not perform that unresolved-execution check. Its proposal context hashes active campaign holds only; activating then clearing a hold can reproduce a pre-hold digest, allowing an otherwise-valid proposal from before the hold to execute. Hosts must prevent restoration during matching unresolved work and invalidate pre-hold campaign proposals until the runtime supplies these safeguards. A fresh proposal and any applicable approval are required by the deployment contract, not guaranteed by this campaign API alone. These two gaps were reproduced with synthetic local fixtures during the diagram consistency review; no runtime fix is included in that documentation change.

Follow-up implementation work: add the campaign unresolved-execution guard and a durable monotonic campaign revision, bind proposals to it, and test restoration across hold activation/clearing, concurrent admission and restart. See [the consistency review](../../Diagram_Source/CONSISTENCY_REVIEW.md).
